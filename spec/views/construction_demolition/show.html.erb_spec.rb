...
describe 'construction_demolition/show', :type => :view do
  ...
  context "when having previously selected 'Yes'" do
    ...
    it "has the matching option selected" do
      assign(:registration, Registration.ctor(attrs={:constructionWaste => 'yes'}))
      render
      expect(rendered).to have_selector('[id=registration_constructionWaste_yes][checked=checked]')
    end
  end
  ...
  context "when having previously selected 'No'" do
    ...
    it "has the matching option selected" do
      assign(:registration, Registration.ctor(attrs={:constructionWaste => 'no'}))
      render
      expect(rendered).to have_selector('[id=registration_constructionWaste_no][checked=checked]')
    end
  end
